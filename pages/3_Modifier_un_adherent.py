from datetime import datetime
import streamlit as st
import pandas as pd
import numpy as np
from lib.database_functions import *

st.set_page_config(
    page_title="ToKool Paddle",
    page_icon="images/icon_paddle.png",
    layout="wide",
    initial_sidebar_state="expanded",
    menu_items={
        'Get Help': None,
        'Report a Bug': None,
        'About': None
    }
)

st.title("Modifier un adhérent")

df = pd.DataFrame(get_adherent_with_relations(), columns=
    [
        "Adherent_id", "Nom", "Prenom", "Date_naissance", "Genre", "Adresse", "Telephone", "Mail", 
        "Date_inscription", "Dossier_medical", "Annee", "Statut", "Description", "Prix"
    ],
)
selected_option = st.selectbox("Sélectionnez un adhérent à modifier :", df["Adherent_id"], format_func=lambda x: df[df["Adherent_id"] == x]["Prenom"].values[0])
record = df.query("Adherent_id == @selected_option")

if record["Prenom"].str != 0:
    with st.form("Formulaire d'ajout d'adhérents"):
        id = value=record['Adherent_id'].values[0].item()
        nom = st.text_input("Nom", value=record['Nom'].values[0])
        prenom = st.text_input("Prenom", value=record['Prenom'].values[0])
        date_naissance = st.date_input("Date de naissance", format="DD/MM/YYYY", min_value=date(1930, 1, 1),  value=datetime.strptime(record['Date_naissance'].values[0], "%Y-%m-%d"))
        options = ("Homme", "Femme")
        genre = st.selectbox("Genre", options, options.index(record['Genre'].values[0]))
        adresse = st.text_input("Adresse", record['Adresse'].values[0])
        telephone = st.text_input("Téléphone", record['Telephone'].values[0])
        mail = st.text_input("E-mail", record['Mail'].values[0])
        inscription_dossier_medical = st.checkbox("Dossier médical fourni ?", value=record['Dossier_medical'].values[0])
        cotisation_statut = st.checkbox("Cotisation réglée ?", value=record['Statut'].values[0])
        if st.form_submit_button("Modifier l'adhérent"):
            update_adherent(id, nom, prenom, date_naissance, genre, adresse, telephone, mail, inscription_dossier_medical, cotisation_statut)
            st.success("L'adhérent a correctement été modifié.")
        
else:
    st.warning("Aucune valeur sélectionnée. Veuillez sélectionner un adhérent à modifier.")